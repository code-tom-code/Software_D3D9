...
const bool IDirect3DTexture9Hook::UpdateTextureInternal(const IDirect3DTexture9Hook* const sourceTexture)
{
	...
	if (InternalUsage & D3DUSAGE_AUTOGENMIPMAP)
	{
		surfaces[0]->UpdateSurfaceInternal(sourceTexture->surfaces[0], NULL, NULL);
		return true;
	}
...
void IDirect3DTexture9Hook::SampleTextureLoD(float x, float y, float mip, const SamplerState& samplerState, D3DXVECTOR4& outColor) const
{
	...
	if (mip == 0.0f)
	{
		surfaces[0]->SampleSurface<writeMask>(x, y, samplerState, outColor);
	}
	else
	{
...
void IDirect3DTexture9Hook::SampleTextureLoD4(float (&x4)[4], float (&y4)[4], float (&mip4)[4], const SamplerState& samplerState, D3DXVECTOR4 (&outColor4)[4]) const
{
	...
	case D3DTEXF_NONE           : // Mip-mapping is disabled, always read level 0 of the surface
	{
		...
		if (!usingBorderColor4[0])
			surfaces[0]->SampleSurface<writeMask>(x4[0], y4[0], samplerState, outColor4[0]);
		if (!usingBorderColor4[1])
			surfaces[0]->SampleSurface<writeMask>(x4[1], y4[1], samplerState, outColor4[1]);
		if (!usingBorderColor4[2])
			surfaces[0]->SampleSurface<writeMask>(x4[2], y4[2], samplerState, outColor4[2]);
		if (!usingBorderColor4[3])
			surfaces[0]->SampleSurface<writeMask>(x4[3], y4[3], samplerState, outColor4[3]);
	}
		break;
	case D3DTEXF_POINT          :
	{
